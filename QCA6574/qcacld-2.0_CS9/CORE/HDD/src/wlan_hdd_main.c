...
static int hdd_netdev_notifier_call(struct notifier_block * nb,
                                         ...
                                         void *ndev)
{
   #if LINUX_VERSION_CODE >= KERNEL_VERSION(3,14,19)
   //3.14.19
...
void hdd_checkandupdate_phymode( hdd_context_t *pHddCtx)
{
   ...
   if (NULL == pAdapter)
   {
       #ifdef WIIF_ONOFF_LOG
       VOS_TRACE(VOS_MODULE_ID_HDD, VOS_TRACE_LEVEL_FATAL,
               "%s,%d,pAdapter is null !!",__func__,__LINE__);
       #endif
       return ;
   }
                             ...
                             const v_BOOL_t bCloseSession)
{
   ...
   switch(pAdapter->device_mode)
   {
      ...
      case WLAN_HDD_P2P_DEVICE:
         ...
         if (pAdapter->device_mode != WLAN_HDD_INFRA_STATION) {
             //LICHENG start
             //This function can reduce wifi power off time at least 500ms
             //See wlan_hdd_cleanup_remain_on_channel_ctx()
...
extern void wifi_card_detect(int on);
#include <linux/of_gpio.h>
void static wifi_set_power(int on)
{
    int  gpio_err = gpio_request(141,"wifi-enable");    
    if (gpio_err >= 0) 
    {
        printk(KERN_EMERG"%s:request wifi enable okkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk\n", __func__);
        gpio_direction_output(141, on);
    }
    else
    {
        printk(KERN_EMERG"%s:request wifi enable faileddddddddddddddddddddddddddddddddddd, gpio_err=%d\n", __func__,gpio_err);
    }
    gpio_set_value(141, on);
}
