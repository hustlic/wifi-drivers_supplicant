...
#define PEER_ALL_TID_BITMASK 0xffffffff
    ...
    #ifdef WIFI_P2P_CONNECT_LOG
	WMA_LOGE("%s,%d: Removed peer with peer_addr %pM vdevid %d peer_count %d",
                    __func__,__LINE__, bssid, vdev_id, wma->interfaces[vdev_id].peer_count);
    #endif
				 ...
				 u_int8_t macaddr[IEEE80211_ADDR_LEN])
{
    ...
    #ifdef WIFI_P2P_CONNECT_LOG
	WMA_LOGE("%s,%d: ID = %d VAP Addr = %02x:%02x:%02x:%02x:%02x:%02x",
                                ...
                                u_int8_t generateRsp)
{
	...
	if (adf_os_atomic_read(&iface->bss_status) == WMA_BSS_STATUS_STARTED) {
        ...
        #ifdef WIFI_P2P_CONNECT_LOG
        WMA_LOGE("%s,%d,Call txrx detach with callback for vdev %d", __func__,__LINE__,vdev_id);
        #endif
		...
		v_BOOL_t roam_synch_in_progress)
{
	...
	if (roam_synch_in_progress) {
        ...
        #ifdef WIFI_P2P_CONNECT_LOG
		WMA_LOGE("%s,%d: Created peer with peer_addr %pM vdev_id %d,"
				"peer_count - %d",__func__, __LINE__,peer_addr, vdev_id,
				wma->interfaces[vdev_id].peer_count);
        #endif
    ...
    #ifdef WIFI_P2P_CONNECT_LOG
	WMA_LOGE("%s,%d: Created peer with peer_addr %pM vdev_id %d, peer_count - %d",
                    __func__, __LINE__,peer_addr, vdev_id, wma->interfaces[vdev_id].peer_count);
    #endif
	...
	*buf = wmi_buf_alloc(wma_handle->wmi_handle, len);
	...
	if (!scan_req->p2pScanType) {
		...
		case P2P_SCAN_TYPE_SEARCH:
			...
			break;
		default:
			WMA_LOGE("%s,%d,Invalid scan type",__func__,__LINE__);
			goto error;
		}
	}
	...
	if (scan_req->numSsid > SIR_SCAN_MAX_NUM_SSID) {
		WMA_LOGE("%s,%d, Invalid value for numSsid",__func__,__LINE__);
		goto error;
	}
...
error:
        vos_mem_free(*buf);
        *buf = NULL;
        printk(KERN_ERR"%s(),%d, Listen failed status is %d\n",__FUNCTION__,__LINE__, vos_status);
        return vos_status;
}
                        ...
                        tSirScanOffloadReq *scan_req, v_U16_t msg_type)
{
	...
	if (status != EOK) {        
		WMA_LOGE("%s,%d,wmi_unified_cmd_send returned Error %d",__func__,__LINE__,
			status);
		vos_status = VOS_STATUS_E_FAILURE;
		goto error;
	}
                                    ...
                                    u_int32_t len)
{
	...
	case WMI_SCAN_EVENT_RESTARTED:
        ...
        if (scan_event->event == (enum sir_scan_event_type)WMI_SCAN_EVENT_COMPLETED) {
				#if 0
				//LICHENG
                WMA_LOGE(" scan complete - scan_id %x, vdev_id %x",wmi_event->scan_id, vdev_id);
				#endif
		/*
...
#define PEER_ALL_TID_BITMASK 0xffffffff
    ...
    #ifdef WIFI_P2P_CONNECT_LOG    
	WMA_LOGA("%s,%d: vdevid %d bssid %pM", __func__, __LINE__,vdev_id, iface->bssid);
    #endif
