 ...
 */
#define BUFFER_NEEDS_BOUNCE(buffer)  (((unsigned long)(buffer) & 0x1FF) || !virt_addr_valid((buffer)))
#else
    ...
    do {
        ...
        if (request & HIF_WRITE) {
            ...
            if (BUFFER_NEEDS_BOUNCE(buffer) && device->dma_buffer != NULL) {
                unsigned long d_offset = (unsigned long ) device->dma_buffer;
                unsigned int mm = 0x200 - (d_offset&0x1FF);
                tbuffer = device->dma_buffer+mm;
                
                
                    /* copy the write data to the dma buffer */
                //AR_DEBUG_ASSERT(length+mm <= HIF_DMA_BUFFER_SIZE);
...
#if HIF_USE_DMA_BOUNCE_BUFFER
    hifdevice->dma_buffer = A_MALLOC(HIF_DMA_BUFFER_SIZE + 512);
