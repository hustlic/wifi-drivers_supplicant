				...
				u_int32_t address, bool compressed)
{
	...
	VOS_TRACE(VOS_MODULE_ID_VOSS, VOS_TRACE_LEVEL_ERROR,
		"%s,%d: transferring file: %s size %d bytes done!", __func__,__LINE__,
...
int ol_download_firmware(struct ol_softc *scn)
{
	...
	//printk("============%s: offset :0x%x\n", __func__, offsetof(struct host_interest_s, hi_board_data));
	printk(KERN_EMERG"============Firstly get  %s: Target address  Using 0x%x\n", __func__, address);
	//address=0;
	for(i = 0; !address && i < 100; i++){
		msleep_interruptible(1);
		BMIReadMemory(scn->hif_hdl,
		host_interest_item_address(scn->target_type, offsetof(struct host_interest_s, hi_board_data)),
		(u_int8_t *)&address, 4, scn);
		printk(KERN_EMERG"============ %s: Retry Target address  Using: 0x%x , Retry count: %d\n", __func__, address,i);
	}
	if (!address) {
		address = AR6004_REV5_BOARD_DATA_ADDRESS;
		printk(KERN_EMERG"%s: Target address not known! Using default 0x%x\n", __func__, address);
	}
	...
	if (status == EOK) {
        ...
        #ifdef WIFI_ONOFF_LOG
		printk("%s,%d: Using 0x%x for the remainder of init\n", __func__, __LINE__,address);
        #endif
...
void ol_pktlog_init(void *hif_sc)
{
    ...
    #ifdef WIFI_ONOFF_LOG
	if (ret)
		pr_err("%s,%d: pktlogmod_init failed ret:%d\n", __func__, __LINE__,ret);
	else
		pr_info("%s,%d: pktlogmod_init successfull\n", __func__,__LINE__);
    #endif
}
#endif
