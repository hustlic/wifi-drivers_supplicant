...
static void wpas_p2p_get_group_ifname(struct wpa_supplicant *wpa_s,
				      char *ifname, size_t len, enum wpa_driver_if_type type)
{
    ...
    //GO always uses 0 as its index.
	if (type == WPA_IF_P2P_GO) {
		os_snprintf(ifname, len, "p2p-%s-0", ifname_ptr);
	} else {
		if (wpa_s->p2p_group_idx == 0) {
			wpa_s->p2p_group_idx++;
		}
					...
					enum wpa_driver_if_type type)
{
	...
	wpas_p2p_get_group_ifname(wpa_s, ifname, sizeof(ifname), type);
	force_ifname[0] = '\0';
		     ...
		     int ht40, int vht)
{
	...
	// redefine go_intent base on p2p_peer's intent
	if (wpa_s->global->p2p) {
		struct p2p_device *dev;
		dev = p2p_get_device(wpa_s->global->p2p, peer_addr);
		if (dev) {
			wpa_printf(MSG_ERROR, "P2P: Redefine go_intent base p2p_peer's intent[%d], previous intent[%d]",
				dev->go_intent, go_intent);
			if (dev->go_intent == 15) {
				go_intent = 14;
			}
		}
...
int wpas_p2p_listen(struct wpa_supplicant *wpa_s, unsigned int timeout)
{
	...
	if (timeout == 0) {
		 ...
		 */
		//Suntec start ++++++++++++++++++++++++++++++++++++++++++++++
		//change to INT MAX, on Goni project, it is 2147483000ms, about 24.86days
		//timeout = 3600;		
