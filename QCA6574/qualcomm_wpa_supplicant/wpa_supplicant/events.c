				...
				struct disassoc_info *info)
{
    ...
    {
        if(suntec_get_disassociate_wait()==0)
        {
            //EAPOL not finished, DUT is not waiting for the disassociate event
            //If we get the disassociate event here, then p2p connect must be failed.
            if(wpa_s->global && wpa_s->global->p2p)
            {
                wpa_dbg(wpa_s, MSG_ERROR, "%s,%d, Dut get disassociate event while p2p connecting, but EAPOL not finished, fail the p2p connect\n"
        ...
        {
            //EAPOL has finished, DUT has sent out the EAPOL-Failure to peer device.
            //The disassociate event is right.
            //According the specification "Wi-Fi_Simple_Configuration_Technical_Specification_v2.0.4-1",Section 6.1
            suntec_set_disassociate_wait(0);
            hostapd_notif_disassoc(wpa_s->ap_iface->bss[0], info->addr);
        }
