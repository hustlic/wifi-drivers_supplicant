...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	{		
        ...
        if (rtw_get_mirror_ie(pframe + WLAN_HDR_A3_LEN + ie_offset, pkt_len - WLAN_HDR_A3_LEN - ie_offset, mirror_ie, &mirror_ielen ))
        {
            DBG_8192C(" yjz mirror ie found!!\n");
        }
#endif
	}
	pstat->p2p_status_code = p2p_status_code;
...
void issue_probersp_p2p(_adapter *padapter, unsigned char *da)
{
	...
	*(fctrl) = 0;
    ...
    wfdielen = build_mirrorlink_ie(pwdinfo, pframe);
	pframe += wfdielen;
	pattrib->pktlen += wfdielen;
...
exit:
	#if 1
	//LICHENG
	printk(KERN_EMERG"LICHENG Func=%s(),Line=%d, ret=%s\n", __FUNCTION__,__LINE__,(ret==_SUCCESS)?("SUCCESS"):("FAIL"));
	#endif
	if (ret == _SUCCESS)
	{
...
#if 0
    DBG_871X("*******%s********\n", __func__);
    for(i=0;i<psurveydone_evt->bss_cnt;i++){
        DBG_871X("ssid=%s\n", pmlmeext->sitesurvey_res.ssid[i].Ssid);
    }
#endif
	DBG_871X(" a survey done event(%x) band:%d for "ADPT_FMT"\n", psurveydone_evt->bss_cnt, padapter->setband, ADPT_ARG(padapter));
