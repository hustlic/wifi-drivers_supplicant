...
#endif
    ret = rtw_get_wps_ie(buf, len, NULL, &wps_ielen);
    printk("***************%s ret=%d wps_ielen=%d**************\n", __func__, ret, wps_ielen);

	#if 1
	//LICHENG 这个return不知道谁加的，加这个return会导致p2p模式下，手机搜不到我们
	//去掉这个return就可以正常搜到了
	#else
	return;
	#endif
