...
void rtw_proc_init_one(struct net_device *dev)
{
	...
	if(rtw_proc == NULL)
	{
		 ...
		 *                                                  ,void *data)
		 */
		struct file_operations proc_fops_2=
		{    		
			.read = proc_get_drv_version,			
		    .owner=THIS_MODULE,
		};
		entry = proc_create_data("ver_info", S_IFREG | S_IRUGO, rtw_proc, &proc_fops_2,dev);		
		#else
		/* TMap 3.0.35 version
		...
		#if 1
		//LICHENG
		struct file_operations proc_fops_3=
		{    		
			.read  = proc_get_log_level,
			.write = proc_set_log_level,
		    .owner = THIS_MODULE,
		};
		entry = proc_create_data("log_level", S_IFREG | S_IRUGO, rtw_proc, &proc_fops_3,dev);
		if (!entry) 
		{
			DBG_871X("Unable to create_proc_read_entry!\n");
			return;
		...
		#if 1
		//LICHENG
		struct file_operations proc_fops_4=
		{    		
			.read  = proc_get_mstat,			
		    .owner = THIS_MODULE,
		};
		entry = proc_create_data("mstat", S_IFREG | S_IRUGO, rtw_proc, &proc_fops_4,dev);
		if (!entry) 
		{
			DBG_871X("Unable to create_proc_read_entry!\n");
			return;
	...
	if(padapter->dir_dev == NULL)
	{
		...
		#if 1
		//LICHENG
		struct file_operations proc_fops_5=
		{    		
		    .owner=THIS_MODULE,
		};
		padapter->dir_dev = proc_mkdir_mode(dev->name, S_IFDIR | S_IRUGO | S_IXUGO, rtw_proc);
		#else
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_6=
	{    		
		.read  = proc_get_write_reg,
		.write = proc_set_write_reg,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("write_reg", S_IFREG | S_IRUGO, dir_dev, &proc_fops_6, dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_7=
	{    		
		.read  = proc_get_read_reg,
		.write = proc_set_read_reg,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("read_reg", S_IFREG | S_IRUGO, dir_dev, &proc_fops_7,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_8=
	{    		
		.read  = proc_get_fwstate,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("fwstate", S_IFREG | S_IRUGO, dir_dev, &proc_fops_8,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_9=
	{    		
		.read  = proc_get_sec_info,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("sec_info", S_IFREG | S_IRUGO, dir_dev, &proc_fops_9,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_10=
	{    		
		.read  = proc_get_mlmext_state,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("mlmext_state", S_IFREG | S_IRUGO, dir_dev, &proc_fops_10,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_11=
	{    		
		.read  = proc_get_qos_option,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("qos_option", S_IFREG | S_IRUGO, dir_dev, &proc_fops_11,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_12=
	{    		
		.read  = proc_get_ht_option,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("ht_option", S_IFREG | S_IRUGO, dir_dev, &proc_fops_12,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_13=
	{    		
		.read  = proc_get_rf_info,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("rf_info", S_IFREG | S_IRUGO, dir_dev, &proc_fops_13,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_14=
	{    		
		.read  = proc_get_ap_info,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("ap_info", S_IFREG | S_IRUGO, dir_dev, &proc_fops_14,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_15=
	{    		
		.read  = proc_get_adapter_state,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("adapter_state", S_IFREG | S_IRUGO, dir_dev, &proc_fops_15,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_16=
	{    		
		.read  = proc_get_trx_info,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("trx_info", S_IFREG | S_IRUGO, dir_dev, &proc_fops_16,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_17=
	{    		
		.read  = proc_get_mac_reg_dump1,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("mac_reg_dump1", S_IFREG | S_IRUGO, dir_dev, &proc_fops_17,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_18=
	{    		
		.read  = proc_get_mac_reg_dump2,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("mac_reg_dump2", S_IFREG | S_IRUGO, dir_dev, &proc_fops_18,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_19=
	{    		
		.read  = proc_get_mac_reg_dump3,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("mac_reg_dump3", S_IFREG | S_IRUGO, dir_dev, &proc_fops_19,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_20=
	{    		
		.read  = proc_get_bb_reg_dump1,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("bb_reg_dump1", S_IFREG | S_IRUGO, dir_dev, &proc_fops_20,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_21=
	{    		
		.read  = proc_get_bb_reg_dump2,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("bb_reg_dump2", S_IFREG | S_IRUGO, dir_dev, &proc_fops_21,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_22=
	{    		
		.read  = proc_get_bb_reg_dump3,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("bb_reg_dump3", S_IFREG | S_IRUGO, dir_dev, &proc_fops_22,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_23=
	{    		
		.read  = proc_get_rf_reg_dump1,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("rf_reg_dump1", S_IFREG | S_IRUGO, dir_dev, &proc_fops_23,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_24=
	{    		
		.read  = proc_get_rf_reg_dump2,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("rf_reg_dump2", S_IFREG | S_IRUGO, dir_dev, &proc_fops_24,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	if((RF_1T2R == rf_type) ||(RF_1T1R ==rf_type ))	
		...
		#if 1
		//LICHENG
		struct file_operations proc_fops_25=
		{    		
			.read  = proc_get_rf_reg_dump3,
		    .owner = THIS_MODULE,
		};
		entry = proc_create_data("rf_reg_dump3", S_IFREG | S_IRUGO, dir_dev, &proc_fops_25,dev);
		if (!entry) 
		{
			DBG_871X("Unable to create_proc_read_entry!\n");
			return;
		...
		#if 1
		//LICHENG
		struct file_operations proc_fops_26=
		{    		
			.read  = proc_get_rf_reg_dump4,
		    .owner = THIS_MODULE,
		};
		entry = proc_create_data("rf_reg_dump4", S_IFREG | S_IRUGO, dir_dev, &proc_fops_26,dev);
		if (!entry) 
		{
			DBG_871X("Unable to create_proc_read_entry!\n");
			return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_27=
	{    		
		.read  = proc_get_all_sta_info,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("all_sta_info", S_IFREG | S_IRUGO, dir_dev, &proc_fops_27,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_28=
	{    		
		.read  = proc_get_malloc_cnt,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("_malloc_cnt", S_IFREG | S_IRUGO, dir_dev, &proc_fops_28,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_29=
	{    		
		.read  = proc_get_best_channel,
		.write = proc_set_best_channel,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("best_channel", S_IFREG | S_IRUGO, dir_dev, &proc_fops_29,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_30=
	{    		
		.read  = proc_get_rx_signal,
		.write = proc_set_rx_signal,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("rx_signal", S_IFREG | S_IRUGO, dir_dev, &proc_fops_30,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_31=
	{    		
		.read  = proc_get_ht_enable,
		.write = proc_set_ht_enable,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("ht_enable", S_IFREG | S_IRUGO, dir_dev, &proc_fops_31,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_32=
	{    		
		.read  = proc_get_cbw40_enable,
		.write = proc_set_cbw40_enable,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("cbw40_enable", S_IFREG | S_IRUGO, dir_dev, &proc_fops_32,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_33=
	{    		
		.read  = proc_get_ampdu_enable,
		.write = proc_set_ampdu_enable,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("ampdu_enable", S_IFREG | S_IRUGO, dir_dev, &proc_fops_33,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_34=
	{    		
		.read  = proc_get_rx_stbc,
		.write = proc_set_rx_stbc,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("rx_stbc", S_IFREG | S_IRUGO, dir_dev, &proc_fops_34,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_35=
	{    		
		.read  = proc_get_two_path_rssi,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("path_rssi", S_IFREG | S_IRUGO, dir_dev, &proc_fops_35,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_36=
	{    		
		.read  = proc_get_vid,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("vid", S_IFREG | S_IRUGO, dir_dev, &proc_fops_36,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_37=
	{    		
		.read  = proc_get_pid,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("pid", S_IFREG | S_IRUGO, dir_dev, &proc_fops_37,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_38=
	{    		
		.read  = proc_get_rssi_disp,
		.write = proc_set_rssi_disp,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("rssi_disp", S_IFREG | S_IRUGO, dir_dev, &proc_fops_38,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_39=
	{    		
		.read  = proc_get_sreset,
		.write = proc_set_sreset,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("sreset", S_IFREG | S_IRUGO, dir_dev, &proc_fops_39,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	...
	#if 1
	//LICHENG
	struct file_operations proc_fops_40=
	{    		
		.read  = proc_get_dm_adaptivity,
		.write = proc_set_dm_adaptivity,
	    .owner = THIS_MODULE,
	};
	entry = proc_create_data("dm_adaptivity", S_IFREG | S_IRUGO, dir_dev, &proc_fops_40,dev);
	if (!entry) 
	{
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
...
int _netdev_vir_if_open(struct net_device *pnetdev)
{
		...
		primary_padapter->hw_init_completed == _TRUE)
	{
		...
		//rtw_start_mbssid_cam(padapter);//start mbssid_cam after bup = _TRUE & hw_init_completed = _TRUE delete by xushengliang
