	...
	struct wpa_driver_associate_params *params)
{
	...
	wpa_printf(MSG_ERROR, "Func=%s(),Line=%d\n", __FUNCTION__,__LINE__);
	wpa_printf(MSG_DEBUG, "  * Auth Type %d", type);
	NLA_PUT_U32(msg, NL80211_ATTR_AUTH_TYPE, type);
...
skip_auth_type:
	if (params->wpa_proto) {
		#if 1
		//LICHENG
		//没有运行到这里，应该是params->wpa_proto没有设置
		#endif
	...
	//LICHENG
		if(params->p2p)
		{
			wpa_printf(MSG_ERROR, "LICHENG  * WPA Versions not set!!!,force to set it for p2p");
			enum nl80211_wpa_versions ver = 0;		
			ver |= NL80211_WPA_VERSION_1;
			ver |= NL80211_WPA_VERSION_2;
	...
	ret = 0;
	wpa_printf(MSG_DEBUG, "LICHENG nl80211: Connect request send successfully");
