					...
					enum wpa_driver_if_type type)
{
	...
	wpa_printf(MSG_ERROR, "LICHENG Func=%s(),Line=%d", __FUNCTION__,__LINE__);
	
	#if 0
	//LICHENG	
	os_snprintf(ifname, sizeof(ifname), "%s", "wlan1");
	#else
...
{
	//运行结果是 6ac0
	wpa_printf(MSG_ERROR, "LICHENG Func=%s(),Line=%d, wpa_s->drv_flags=%x\n"
	...
	{
		//LICHENG
		wpa_printf(MSG_ERROR, "LICHENG Func=%s(),Line=%d\n", __FUNCTION__,__LINE__);
		     ...
		     int ht40)
{
	...
	//LICHENG
	wpa_printf(MSG_ERROR, "LICHENG Func=%s(),Line=%d, auth=%d, wpa_s->own_addr=%x:%x:%x:%x:%x:%x"
	...
	if (wpa_s->create_p2p_iface) {
		wpa_printf(MSG_ERROR, "LICHENG Func=%s(),Line=%d, create new interface \n", __FUNCTION__,__LINE__);
