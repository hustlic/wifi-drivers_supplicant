...
int LICHENG_strnicmp(const char *s1, const char *s2, size_t len)
{
...
int rtw_android_cmdstr_to_num(char *cmdstr)
{
	...
	for(cmd_num=0 ; cmd_num<ANDROID_WIFI_CMD_MAX; cmd_num++)
		#if 1
		//LICHENG
		if(0 == LICHENG_strnicmp(cmdstr , android_wifi_cmd_str[cmd_num], strlen(android_wifi_cmd_str[cmd_num])) )
		#else
...
int rtw_android_priv_cmd(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	{
        printk("******************%s: ANDROID_WIFI_CMD_SET_AP_WPS_P2P_IE*****************\n", __func__);
