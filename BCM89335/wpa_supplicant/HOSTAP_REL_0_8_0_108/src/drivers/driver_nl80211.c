...
#ifdef ANDROID_P2P
		/* Ideally nl_recvmsgs is supposed to send 0 on success and non-zero values on
		 * failures. But Android version of libnl seems to return msglen which
		 * is wrongly printed as error.
		 */
		if (res < 0) {
#else
		if (res) {
...
static int nl80211_vendor_reply_brcm(struct nl80211_vendor_data *vendor,
		void *data, int len)
{
	struct wpa_driver_nl80211_data *drv = vendor->nl80211_data;

	wpa_printf(MSG_INFO, "nl80211: BRCM Vendor handler");

	switch (vendor->scmd) {
		case BRCM_VENDOR_SUBCMD_PRIV_STR:
			if (len)
				wpa_msg(drv->ctx, MSG_INFO, "%s", (char *)data);
			else
				wpa_printf(MSG_INFO, "nl80211: Invalid len for Vendor data");
			break;

		default:
				wpa_printf(MSG_INFO, "nl80211: Unsupported Sub command");
	}

	return 0;
}

static int nl80211_vendor_reply_handler(struct nl_msg *msg, void *arg)
{
	char *data = NULL;
	int len = 0;
	struct nl80211_vendor_data *vendor = (struct nl80211_vendor_data *)arg;
	struct nlattr *tb_msg[NL80211_ATTR_MAX + 1];
	struct genlmsghdr *gnlh = nlmsg_data(nlmsg_hdr(msg));

	wpa_printf(MSG_INFO, "nl80211: Vendor command reply handler");
	...
	if (tb_msg[NL80211_ATTR_VENDOR_DATA]) {
		...
		data = nla_data(tb_msg[NL80211_ATTR_VENDOR_DATA]);
		len = nla_len(tb_msg[NL80211_ATTR_VENDOR_DATA]);
		wpa_printf(MSG_INFO, "nl80211: Supported vendor cmd: data_ptr=0x%p len=%u",
				data, len);
	}

	switch(vendor->id) {
		case OUI_BRCM:
			nl80211_vendor_reply_brcm(vendor, data, len);
			break;

		default:
			wpa_printf(MSG_ERROR,
				"nl80211: No handlers for vendor OUI (0x%x)", vendor->id);
	}

	return NL_SKIP;
}

int nl80211_send_vendor_command(struct wpa_driver_nl80211_data *drv ,
				struct i802_bss *bss, int sub_cmd, char *cmd_ptr,
				size_t len, int vendor_id)
{
	struct nl_msg *msg;
	int ret = -1;
	struct nl80211_vendor_data vendor;

	msg = nlmsg_alloc();
	if (!msg)
		return -ENOMEM;

	vendor.nl80211_data = drv;
	vendor.id = vendor_id;
	vendor.scmd = sub_cmd;
	vendor.cmd_data = cmd_ptr;
	vendor.cmd_data_len = len;
	...
	NLA_PUT_STRING(msg, NL80211_ATTR_VENDOR_DATA, cmd_ptr);

	ret = send_and_recv_msgs(drv, msg, nl80211_vendor_reply_handler, (void *)&vendor);
	msg = NULL;
    if (ret) {
...
#ifdef ANDROID_P2P
	if (wpa_driver_priv_lib_init(bss) < 0) {
		wpa_printf(MSG_ERROR,
				"nl80211: private lib initilization failed\n");
		goto failed;
	}
#endif /* ANDROID_P2P */
...
#ifdef ANDROID_P2P
    if (wpa_driver_priv_lib_deinit(bss) < 0) {
        wpa_printf(MSG_ERROR,
