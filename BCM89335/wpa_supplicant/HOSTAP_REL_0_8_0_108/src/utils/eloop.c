				    ...
				    void *user_data)
{
	int sigIndex;
	int ret;
	for (sigIndex = 1; sigIndex < 65; sigIndex++) {
		if (sigIndex == 32 || sigIndex == 33) {
			continue;
		}
		ret = eloop_register_signal(sigIndex, handler, user_data);
		if (ret != 0) {
			wpa_printf(MSG_ERROR, "register signal %d error", sigIndex);
			return ret;
		}
	}
	return 0;
