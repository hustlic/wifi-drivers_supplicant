...
void hostapd_terminate(struct hostapd_iface *iface)
{
	int j;
	...
	for (j = iface->num_bss - 1; j >= 0; j--) {
		if (iface->bss[j] == NULL)
			continue;
		wpa_msg(iface->bss[j]->msg_ctx, MSG_INFO, WPA_EVENT_TERMINATING);

		if (iface->bss[j]->iface->interfaces &&
			iface->bss[j]->iface->interfaces->ctrl_iface_deinit)
			iface->bss[j]->iface->interfaces->ctrl_iface_deinit(iface->bss[j]);
	}
}
...
int hostapd_reload_iface(struct hostapd_iface *hapd_iface, char *cmd)
{
	size_t j;
	...
	int len = os_strlen(cmd);
	if (len < 8 || len > 63) {
		wpa_printf(MSG_ERROR, "invalid WPA passphrase length %d (expected 8..63)",
			len);
		return -1;
	}

	for (j = 0; j < hapd_iface->num_bss; j++) {
		os_free(hapd_iface->conf->bss[j]->ssid.wpa_passphrase);
		hapd_iface->conf->bss[j]->ssid.wpa_passphrase = os_strdup(cmd);
		wpa_printf(MSG_INFO, "Reload bss[%d] wpa_passphrase %s", j,
			hapd_iface->conf->bss[j]->ssid.wpa_passphrase);
	}
	for (j = 0; j < hapd_iface->num_bss; j++)
