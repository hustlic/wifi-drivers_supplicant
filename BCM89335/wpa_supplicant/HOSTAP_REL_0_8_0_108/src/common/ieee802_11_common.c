					    ...
					    int show_errors)
{
	...
	case OUI_MIRROR_LNK:
		...
		case WFD_OUI_TYPE:
			elems->mrlnk = pos;
			elems->mrlnk_len = elen;
			wpa_printf(MSG_MSGDUMP, "mirrorlink ie found in packet%s len:%d\n", pos, elen);
			break;
		default:
			wpa_printf(MSG_EXCESSIVE, "Unknown MIRROR_LINK IE IGORED"
					"type=%d len=%lu", pos[3], (unsigned long)elen);
			return -1;
		}
#endif
	default:
					    ...
					    u32 oui_type)
{
	...
	wpa_printf(MSG_DEBUG, "%s length:%d\n", __func__, ies_len);
