...
#ifdef CONFIG_MIRROR_LINK_IE
	if (group->p2p->mrlnk_ie)
		extra += wpabuf_len(group->p2p->mrlnk_ie);
#endif
...
#ifdef CONFIG_MIRROR_LINK_IE
	if (group->p2p->mrlnk_ie)
		wpabuf_put_buf(ie, group->p2p->mrlnk_ie);
...
#ifdef CONFIG_MIRROR_LINK
struct wpabuf * wifi_display_mrlnk_encaps(struct wpabuf *subelems)
{
	struct wpabuf *ie;
	const u8 *pos, *end;
...
#ifdef CONFIG_MIRROR_LINK_IE
	if (group->p2p->mrlnk_ie) {
		struct wpabuf *mrlnk_ie = wpabuf_dup(group->p2p->mrlnk_ie);
		ie = wpabuf_concat(mrlnk_ie, ie);
	}
#endif /* CONFIG_MIRROR_LINK_IE */
...
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_MIRROR_LINK_IE
	m->mrlnk_ie = ieee802_11_vendor_ie_concat(ie, len, MIRRORLNK_IE_VENDOR_TYPE);
#endif /* CONFIG_WIFI_DISPLAY */
...
#ifdef CONFIG_MIRROR_LINK
	if (group->mrlnk_ie)
		extra += wpabuf_len(group->mrlnk_ie);
#endif
...
#ifdef CONFIG_MIRROR_LINK
	if (group->mrlnk_ie)
		wpabuf_put_buf(resp, group->mrlnk_ie);
#endif /* CONFIG_WIFI_DISPLAY */
