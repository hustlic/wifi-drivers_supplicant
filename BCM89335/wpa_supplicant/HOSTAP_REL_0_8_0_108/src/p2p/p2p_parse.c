...
int p2p_parse_ies(const u8 *data, size_t len, struct p2p_message *msg)
{
	...
	if (elems.mrlnk) {
		msg->mrlnk_subelems = ieee802_11_vendor_ie_concat(
				data, len, MIRRORLNK_IE_VENDOR_TYPE);
		wpa_hexdump_buf(MSG_MSGDUMP, "mirror: data", msg->mrlnk_subelems);
	}
#endif
...
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_MIRROR_LINK_IE
	wpabuf_free(msg->mrlnk_subelems);
	msg->mrlnk_subelems = NULL;
