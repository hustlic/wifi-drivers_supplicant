...
#ifdef BCM_SDO
	/* Run expiry logic only if device discovery offload is NOT enabled */
	if (!p2p->cfg->ddo_enabled)
#endif /* BCM_SDO */
...
#ifdef CONFIG_MIRROR_LINK_IE
	if (msg.mrlnk_subelems) {
		wpabuf_free(dev->info.mrlnk_subelems);
		dev->info.mrlnk_subelems = wpabuf_dup(msg.mrlnk_subelems);
	}
#endif
...
static int p2p_get_next_prog_freq_5g(struct p2p_data *p2p)
{
	...
	for (cl = 1; cl < c->reg_classes; cl++) {
		cla = &c->reg_class[cl];
		if (cla->reg_class != p2p->last_prog_scan_class_5g)
			continue;
		for (ch = 0; ch < cla->channels; ch++) {
			if (cla->channel[ch] == p2p->last_prog_scan_chan_5g) {
				found = 1;
				break;
			}
		}
	...
	if (!found) {
		/* Start from beginning */
		reg_class = c->reg_class[1].reg_class;
		channel = c->reg_class[1].channel[0];
	} else {
		/* Pick the next channel */
		ch++;
		if (ch == cla->channels) {
	...
	p2p_dbg(p2p, "Next progressive search channel: reg_class %u channel %u -> %d MHz",
		reg_class, channel, freq);
	p2p->last_prog_scan_class_5g = reg_class;
	p2p->last_prog_scan_chan_5g = channel;
...
static void p2p_search(struct p2p_data *p2p)
{
	...
	if (p2p->find_type == P2P_FIND_PROGRESSIVE &&
		...
		p2p_dbg(p2p, "Starting search (+ freq %u)", freq[i]);
	}
	if (p2p->find_type == P2P_FIND_FAST_PROGRESSIVE ) {
		if (((freq[i] = p2p_get_next_prog_freq(p2p)) > 0)  && (freq[i] <= 2462) ) {
			p2p_dbg(p2p, "Starting search (+ freq %u)", freq[i]);
			i++;
			for (j = 1; j < MAX_FREQ_5G; j++) {
				if ((freq[i] = p2p_get_next_prog_freq_5g(p2p)) > 0)
					i++;
			}
			type = P2P_SCAN_SOCIAL_PLUS_5G;
			p2p_dbg(p2p, "Starting search (+ freq 5g = %u %u %u )", freq[1], freq[2], freq[3]);
		}
	}
#else
	if (p2p->find_type == P2P_FIND_PROGRESSIVE &&
...
int p2p_connect(struct p2p_data *p2p, const u8 *peer_addr,
		...
		int pd_before_go_neg, unsigned int pref_freq, u16 oob_pw_id, int reject)
{
	struct p2p_device *dev;
...
#ifdef PSET_P2P_SET_GO_PASSPHRASE
    memcpy(params->passphrase, p2p->passphrase, 8);
#else
    p2p_random(params->passphrase, 8);
#endif /* PSET_P2P_SET_GO_PASSPHRASE */
	return 0;
}
...
#ifdef PSET_P2P_SET_GO_PASSPHRASE
        memcpy(res.passphrase, p2p->passphrase, 8);
#else
...
#ifdef CONFIG_MIRROR_LINK_IE
	if (p2p->mrlnk_ie) {
		wpa_printf(MSG_DEBUG, "swa build probe_resp:%s\n", __func__);
		extra += wpabuf_len(p2p->mrlnk_ie);
		wpa_printf(MSG_DEBUG, "swa build probe_resp:%s extra:%d buf->used:%d\n", __func__, extra, p2p->mrlnk_ie->used);
	}
#endif
...
#ifdef CONFIG_MIRROR_LINK_IE
	if (p2p->mrlnk_ie) {
		wpa_printf(MSG_DEBUG, "swa:%s build probe_resp:%s\n", __func__,  p2p->cfg->dev_name);
		wpa_hexdump(MSG_MSGDUMP, "MIRROR LINK IE for probe resp", p2p->mrlnk_ie, p2p->mrlnk_ie->size);
		wpabuf_put_buf(buf, p2p->mrlnk_ie);
		wpa_hexdump(MSG_DEBUG, "MIRROR LINK IE for probe resp", buf, (1000+extra));
		wpa_printf(MSG_DEBUG, "swa MIRR lnk :%p\n", &p2p->mrlnk_ie);
	}
#endif
	/* P2P IE */
...
#ifdef CONFIG_MIRROR_LINK_IE
		if (p2p->mrlnk_ie)
			wpabuf_put_buf(tmp, p2p->mrlnk_ie);
#endif
...
#endif
#ifndef PSET_P2P_DEVICE_CAPABILITY
...
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_MIRROR_LINK_IE
	wpabuf_free(p2p->mrlnk_ie);
	wpabuf_free(p2p->mrlnk_ie_upnp);
	wpabuf_free(p2p->mrlnk_ie_internet_access);
#endif
...
#ifdef CONFIG_MIRROR_LINK_IE
	if (p2p->mrlnk_ie)
		wpabuf_put_buf(ies, p2p->mrlnk_ie);
		wpa_printf(MSG_DEBUG, "swa%s MIRR lnk :%p\n",__func__, &p2p->mrlnk_ie);
#endif
...
#if defined(BCM_SDO)
int p2p_get_op_channel(struct p2p_data *p2p)
{
	u8 channel;
	p2p_prepare_channel_best(p2p);
	channel = p2p->op_channel;
	wpa_printf(MSG_DEBUG, "%s: op_channel:%d\n",__func__, channel);
	return channel;
}
#endif
...
int p2p_set_pref_freq_2g_5g(struct p2p_data *p2p, int pref_2g, int pref_5g)
{
	p2p_dbg(p2p, "Set p2p pref 5G %u 2G  %u channels",
		 pref_5g, pref_2g);
	p2p->cfg->pref_freq_2g = pref_2g;
	p2p->cfg->pref_freq_5g = pref_5g;
	return 0;
}
#endif
		      ...
		      const struct p2p_channel *pref_chan)
{
    ...
    p2p_dbg(p2p, "Set prefer channel: reg_class %u channel %u",
                p2p->cfg->pref_chan->op_class, p2p->cfg->pref_chan->chan);
	return 0;
}
...
#ifdef CONFIG_MIRROR_LINK_IE
int p2p_set_mirror_link_ie(struct p2p_data *p2p, struct wpabuf *ie)
{
	wpabuf_free(p2p->mrlnk_ie);
	p2p->mrlnk_ie = ie;
	p2p_update_wfd_ie_groups(p2p);
	wpa_hexdump_buf(MSG_DEBUG, "MIRROR LINK IE for ", p2p->mrlnk_ie);
	return 0;
}
int p2p_set_mirror_link_upnp_subelem(struct p2p_data *p2p, struct wpabuf *e)
{
	wpabuf_free(p2p->mrlnk_ie_upnp);
	if (e) {
		p2p->mrlnk_ie_upnp = wpabuf_dup(e);
		if (p2p->mrlnk_ie_upnp == NULL)
			return -1;
	} else {
		p2p->mrlnk_ie_upnp = NULL;
		return 0;
	}


	wpa_hexdump_buf(MSG_DEBUG, "MIRROR LINK IE for  upnp subelem ", p2p->mrlnk_ie_upnp);
	return 0;
}
int p2p_set_mirror_link_internet_access_subelem(struct p2p_data *p2p, struct wpabuf *e)
{
	wpabuf_free(p2p->mrlnk_ie_internet_access);
	if (e) {
		p2p->mrlnk_ie_internet_access = wpabuf_dup(e);
		if (p2p->mrlnk_ie_internet_access == NULL)
			return -1;
	} else
		p2p->mrlnk_ie_internet_access= NULL;


	wpa_hexdump_buf(MSG_DEBUG, "MIRROR LINK IE for internet access subelem ", p2p->mrlnk_ie_internet_access);
	return 0;
}
#endif
...
#ifdef PSET_P2P_SET_GO_PASSPHRASE
int p2p_set_go_passphrase(struct p2p_data *p2p, const char* passphrase)
{
    if (passphrase) {
        os_memcpy(p2p->passphrase, passphrase, 8);
        return 0;
    }
    return -1;
}
...
#endif /* BCM_GENL */
#if defined(BCM_SDO)
int p2p_get_listen_channel(struct p2p_data *p2p)
{
	u8 channel;

	channel = p2p->cfg->channel;
	wpa_printf(MSG_DEBUG, "%s: listen_channel:%d\n",__func__, channel);
	return channel;
}
#endif
#ifdef BCM_SDO
struct wpabuf *p2p_get_probe_resp_ies(struct p2p_data *p2p)
{
        struct wpabuf *ies = NULL;
		int roll_back = 0;

	/* While setting via out of band mechanism enforce the
	 * SD discovery part also which other wise is disabed
	 */
	if(!(p2p->dev_capab & P2P_DEV_CAPAB_SERVICE_DISCOVERY)) {
		roll_back = 1;
		p2p->dev_capab |= P2P_DEV_CAPAB_SERVICE_DISCOVERY;
	}
...
int p2p_enable_dd_offload(struct p2p_data *p2p, u8 enable)
{
	if (!p2p)
		return -EINVAL;
	...
	if (enable) {
		...
		/* On FIND/LISTEN STOP, update the age and clear reported flag */
		struct p2p_device *dev;
		dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
			dev->flags &= ~P2P_DEV_REPORTED;
		...
		wpa_printf(MSG_DEBUG,
			 "P2P_DEV_REPORTED flag cleared");
	}

	p2p->cfg->ddo_enabled = enable;

	wpa_printf(MSG_DEBUG, "P2P: Device Discovery offload %s", enable ? "Enabled" : "Disabled");
