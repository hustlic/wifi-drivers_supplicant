...
int wlmMcsRateSet(WLM_MCS_RATE mcs_rate, WLM_STF_MODE stf_mode)
{
    ...
    printf("wlm: %s, mcs_rate=%d, stf_mode=%d\n", __FUNCTION__, mcs_rate, stf_mode);
	if (mcs_rate > 32) {
...
int wlmOutStart()
{
    ...
    if (wlu_set(irh, WLC_OUT, NULL, 0) < 0) {
        printf("wlmEnableCarrierTone: set WLC_OUT %s\n", wlmLastError());
        return FALSE;
    }
    return TRUE;
}
...
int wlmEnableCarrierTone(int enable, int channel)
{
    ...
    printf("wlm: %s, enable=%d, channel=%d\n", __FUNCTION__, enable, channel);
	if (!enable) {
		val = 0;
	}
	else {
...
int wlmEnableEVMTest(int enable, WLM_RATE rate, int channel)
{
	int val[3] = {0};
        ...
        printf("wlm: %s, enable=%d, rate=%d, channel=%d\n", __FUNCTION__, enable, rate, channel);
...
int wlmTxPacketStartWithMac(unsigned int interPacketDelay,
	unsigned int numPackets, unsigned int packetLength,
	const char* destMac, const char* srcMac, int withAck, int syncMode)
{
	wl_pkteng_t pkteng;

        printf("wlm: %s, interPacketDelay=%d, numPackets=%d, packetLength=%d, destMac=%s, srcMac=%s, withack=%d, syncmode=%d\n",
            __FUNCTION__, interPacketDelay, numPackets, packetLength, destMac, srcMac, withAck, syncMode);
        ...
        if (srcMac != NULL) {
        	if (!wl_ether_atoe(srcMac, (struct ether_addr *)&pkteng.src)) {
        		printf("wlmTxPacketStart: srcMac invalid\n");
        		return FALSE;
        	}
        }
...
int wlmRxPacketStartWithMac(const char* srcMac, const char* dstMac, int withAck,
	int syncMode, unsigned int numPackets, unsigned int timeout)
{
	wl_pkteng_t pkteng;

        printf("wlm: %s, srcMac=%s, dstMac=%s, withAck=%d, syncMode=%d, numPackets=%d, timeout=%d\n",
            __FUNCTION__, srcMac, dstMac, withAck, syncMode, numPackets, timeout);
        ...
        if (dstMac != NULL) {
        	if (!wl_ether_atoe(dstMac, (struct ether_addr *)&pkteng.dest)) {
        		printf("wlmRxPacketStart: dstMac invalid\n");
        		return FALSE;
        	}
       }
...
#else
    // we force to receive the unicast packets
    *count = dtoh32(cnt->pktengrxducast);
    mcastcnt = dtoh32(cnt->pktengrxdmcast);
#endif    
    printf("wlm: %s, pktengrxdmcast=%d, pktengrxducast=%d\n", __FUNCTION__, mcastcnt, *count);
	return TRUE;
}
...
int wlmPHYForceCalSet(int val)
{
    printf("wlm: %s\n", __FUNCTION__);
	if (wlu_iovar_setint(irh, "phy_forcecal", val) < 0) {
