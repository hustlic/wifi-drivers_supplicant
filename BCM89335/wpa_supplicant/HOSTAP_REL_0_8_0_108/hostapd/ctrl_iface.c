...
static int hostapd_ctrl_iface_reload(struct hostapd_iface *iface, char *cmd)
{
	if (hostapd_reload_iface(iface, cmd) < 0) {
				       ...
				       void *sock_ctx)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (hostapd_ctrl_iface_reload(hapd->iface, buf + 7))
			reply_len = -1;
#ifdef PSET_AP_ADD_DRIVER_CMD
#if defined(ANDROID) || defined(BCM_LINUX_BUILD)
    } else if (os_strncmp(buf, "DRIVER ", 7) == 0) {
        reply_len = hostapd_driver_cmd(hapd, buf + 7, reply, reply_size);
#endif
#endif
