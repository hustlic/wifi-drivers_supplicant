				  ...
				  struct hostapd_config *conf)
{
	...
	if (wpa_s->conf->p2p_pref_freq_2g && wpa_s->conf->p2p_pref_freq_5g) {
		...
		if (num > 0) {
			if (freq > 2484) {
				/* STA freq is in 5G, choose 2.4 G */
				wpa_printf(MSG_ERROR, "Choosing 2.4G for AP mod");
				ssid->frequency = wpa_s->conf->p2p_pref_freq_2g;
			} else {
				wpa_printf(MSG_ERROR, "Choosing 5G for AP mode");
				ssid->frequency = wpa_s->conf->p2p_pref_freq_5g;
			}
		} else {
			wpa_printf(MSG_ERROR, "No shared frequency. "
						"Start AP on default channel");
			ssid->frequency = 2412;
		}
	}
#endif
