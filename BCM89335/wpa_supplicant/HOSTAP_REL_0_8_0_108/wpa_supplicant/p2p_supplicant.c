...
#ifdef BCM_SDO
void wpas_p2p_scan_res_handler(struct wpa_supplicant *wpa_s,
...
#endif
{
	...
	case P2P_SCAN_SOCIAL_PLUS_5G:
		...
		params->freqs[1] = 2437;
		params->freqs[2] = 2462;
		for ( i = 0; i < MAX_FREQ_5G; i++) {
			params->freqs[i+3] = freq[i];
		}
		params->freqs[i+3] = 0;
		break;
#endif
			   ...
			   int new_device)
{
	...
	char *mirror_link_info_hex = NULL;
#ifdef CONFIG_WIFI_DISPLAY
	wfd_dev_info_hex = wifi_display_subelem_hex(info->wfd_subelems,
						    WFD_SUBELEM_DEVICE_INFO);
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_MIRROR_LINK_IE
	mirror_link_info_hex = wifi_mirror_link_subelem_hex(info->mrlnk_subelems);
#endif

#ifdef PSET_P2P_SCAN_RESULT
	wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR
		       " p2p_dev_addr=" MACSTR
		       " pri_dev_type=%s name='%s' config_methods=0x%x level=%d "
		       "dev_capab=0x%x group_capab=0x%x%s%s%s%s"
			   ,
		       MAC2STR(addr), MAC2STR(info->p2p_device_addr),
		       wps_dev_type_bin2str(info->pri_dev_type, devtype,
					    sizeof(devtype)),
		       info->device_name, info->config_methods, info->level,
		       info->dev_capab, info->group_capab,
		       wfd_dev_info_hex ? " wfd_dev_info=0x" : "",
		       wfd_dev_info_hex ? wfd_dev_info_hex : "",
			mirror_link_info_hex ? " mirror_link_info_hex=0x" : "",
			mirror_link_info_hex ?  mirror_link_info_hex : ""
		);
#else
	wpa_msg_global(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR
		       " p2p_dev_addr=" MACSTR
...
#endif /* PSET_P2P_SCAN_RESULT */

	os_free(wfd_dev_info_hex);
	os_free(mirror_link_info_hex);
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
...
#ifdef BRCM_DEBUG
	if(os_strncmp(wpa_s->ifname, "wlan0", 5) == 0) {
		 /*
		  * P2P initialization shouldn't happen on wlan0 interface.
		  * For kernels < 3.8, this should happen on p2p0 interface and for
		  * kernels > 3.8, it should happen on p2p-dev-wlan0 (dedicated p2p
		  * discovery interface). This check is purely for debugging purposes
		  * and will be mogrified for external releases.
		 */
		wpa_printf(MSG_ERROR, "BRCMDBG: Wrong initialization sequence (p2p "
			"intialization on wlan0) Supplicant initialization Failed! ");
	...
	if (wpa_s->conf->p2p_pref_freq_5g && wpa_s->conf->p2p_pref_freq_2g) {
		global->p2p_pref_freq_5g = wpa_s->conf->p2p_pref_freq_5g;
		global->p2p_pref_freq_2g = wpa_s->conf->p2p_pref_freq_2g;
	} else
		wpa_printf(MSG_ERROR, "P2P: Failed to configure supported "
				 "freq's for 2G/5G");
 #endif
...
static int wpas_p2p_start_go_neg(struct wpa_supplicant *wpa_s,
				 ...
				 struct wpa_ssid *ssid, unsigned int pref_freq, int reject)
{
				...
				int *force_freq, int *pref_freq, int go)
{
	...
	if (wpa_s->conf->p2p_pref_freq_5g && wpa_s->conf->p2p_pref_freq_2g) {
	/*
	 * This would get mogrified for base build of supplicant. This is an interim solution
	 * for Demo purposes.
	 */
		if (num == 1) {
		/* If there are more than one shared radios don't do any optimization */
			if (freqs[0] > 2482) {
				wpa_printf(MSG_ERROR, "STA using 2.4G. Use 5G as P2P preferred channel");
				*pref_freq = wpa_s->conf->p2p_pref_freq_5g;
			} else {
				wpa_printf(MSG_ERROR, "STA using 5G. Use 2.4G as P2P preferred channel");
				*pref_freq = wpa_s->conf->p2p_pref_freq_2g;
			}
			if (*pref_freq && p2p_supported_freq(wpa_s->global->p2p, *pref_freq)) {
				wpa_printf(MSG_ERROR, "P2P supported on the preferred channel");
				goto exit_ok;
			} else {
				wpa_printf(MSG_ERROR, "P2P NOT supported on the preferred channel");
				*pref_freq = 0;
			}
		}
	}
#endif /* defined(CONFIG_BRCM_RSDB) */
		     ...
		     int ht40, int vht, int reject)
{
	...
	if (wpas_p2p_start_go_neg(wpa_s, peer_addr, wps_method,
				  ...
				  persistent_group, ssid, pref_freq, reject) < 0) {
		if (wpa_s->create_p2p_iface)
			wpas_p2p_remove_pending_group_interface(wpa_s);
		return -1;
	}
		       ...
		       int freq, int ht40, int vht)
{
	...
	if (wpa_s->conf->ago_passphrase)
		os_strncpy((char *)&params.passphrase, wpa_s->conf->ago_passphrase, 8);
#endif
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_FREQ_2G_5G) {
		...
		if (p2p_set_pref_freq_2g_5g(p2p, wpa_s->conf->p2p_pref_freq_5g,
					wpa_s->conf->p2p_pref_freq_2g) < 0) {
			wpa_printf(MSG_ERROR, "P2P: Preferred channel list in 2G 5G "
				"update failed");
...
#ifdef BCM_GENL
int wpas_expire_p2p_peer(struct wpa_supplicant *wpa_s, u8 *addr)
{
	return p2p_remove_peer(wpa_s->global->p2p, addr);
}
#endif /* BCM_GENL */

#ifdef BCM_SDO
int wpas_set_probe_resp_ies(struct wpa_supplicant *wpa_s)
{
	int ret = 0;
	struct wpabuf *ies;
	ies = p2p_get_probe_resp_ies(wpa_s->global->p2p);
	if(!ies) {
	...
	if(wpa_drv_set_ap_wps_ie(wpa_s, NULL, ies, NULL) < 0) {
		wpa_printf(MSG_ERROR,
                        "P2P: Failed to set ies");
		ret = -1;
	}
...
int wpas_enable_dd_offload(struct wpa_supplicant *wpa_s, u8 enable)
{
	return p2p_enable_dd_offload(wpa_s->global->p2p, enable);
}

int wpas_get_listen_channel (struct wpa_supplicant *wpa_s)
{
	return p2p_get_listen_channel(wpa_s->global->p2p);
}
#endif /* BCM_SDO */
...
int wpas_p2p_set_go_passphrase(struct wpa_supplicant *wpa_s, const char* passphrase)
{
	if (wpa_s->global->p2p_disabled || wpa_s->global->p2p == NULL)
		return -1;
	return p2p_set_go_passphrase(wpa_s->global->p2p, passphrase);
}
