			      ...
			      enum wpa_states state)
{
	...
	if ((state == WPA_DISCONNECTED) &&
		...
		char buf[64];
		wpa_drv_driver_cmd(wpa_s, "SETROAMOFFLOAD 0", buf, sizeof(buf));
		wpa_s->drv_flags &= ~WPA_DRIVER_FLAGS_4WAY_HANDSHAKE;
		wpa_dbg(wpa_s, MSG_INFO, "SETROAMOFFLOAD disabled");
	}
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	if (os_strcmp(wpa_s->ifname, "wlan0") == 0) {
		int err;
		char buf[64];
		int supported_security = WPA_KEY_MGMT_PSK |
			WPA_KEY_MGMT_FT_PSK | WPA_KEY_MGMT_FT_IEEE8021X |
			WPA_KEY_MGMT_CCKM;
		int try_okc = ssid->proactive_key_caching < 0 ?
			     wpa_s->conf->okc : ssid->proactive_key_caching;
		try_okc = try_okc && (ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X);
		wpa_dbg(wpa_s, MSG_INFO, "ASSOC KEY MGMT 0x%X\n", ssid->key_mgmt);
		/* support WPA/WPA2-PSK, FT-PSK, 802.1x FT, 802.1x OKC, CCKM */
		if (try_okc || (ssid->key_mgmt & supported_security)) {
			if (!(err = wpa_drv_driver_cmd(wpa_s, "SETROAMOFFLOAD 1", buf, sizeof(buf)))) {
				wpa_s->drv_flags |= WPA_DRIVER_FLAGS_4WAY_HANDSHAKE;
				wpa_dbg(wpa_s, MSG_INFO, "ROAMOFFLOAD enabled");
			}
		} else {
			wpa_s->drv_flags &= ~WPA_DRIVER_FLAGS_4WAY_HANDSHAKE;
			wpa_drv_driver_cmd(wpa_s, "SETROAMOFFLOAD 0", buf, sizeof(buf));
			wpa_dbg(wpa_s, MSG_INFO, "ROAMOFFLOAD disabled");
		}
	}
#endif /* BRCM_ROAM_OFFLOAD */
...
#ifdef BRCM_DRV_ROAM
	char buf[10];
	/* Insert roam preferences only during initial association or when network
	 * configuration has changed.
	 */
	if (wpas_driver_bss_selection(wpa_s) &&
		(((wpa_s->current_ssid == NULL) && wpa_s->disconnect_reason == 0) ||
		(wpa_s->current_ssid && wpa_s->current_ssid != ssid))) {
		wpa_msg(wpa_s, MSG_DEBUG, "Configure roam preferences in driver");
		if (wpa_drv_driver_cmd(wpa_s, "SET_ROAMPREF", buf, sizeof(buf)) < 0) {
			wpa_printf(MSG_DEBUG, "SET_ROAMPREF failed");
		}
	}
#endif /* BRCM_DRV_ROAM */
}
	...
	if ( (ssid->wapi) || (ssid->proto & WPA_PROTO_WAPI) || (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK ) ||
		...
		if (bss) {
			...
			if(ie) {
				...
				snprintf((char *) params.ssid, ((ie[1]>MAX_SSID_LEN)?MAX_SSID_LEN:ie[1])+1, "%s", ie ? wpa_ssid_txt(ie + 2, ie[1]) : "");
...
#ifdef BRCM_DRV_ROAM
	/* Firmware generates the RSN IE for driver based roaming */
	if (wpas_driver_bss_selection(wpa_s)) {
		params.wpa_ie = NULL;
		params.wpa_ie_len = 0;
	} else {
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	if (wifi_mirror_link_init(global) < 0) {
		wpa_printf(MSG_ERROR, "Failed to initialize MIRROR LINK");
		wpa_supplicant_deinit(global);
		return NULL;
	}
#endif /* CONFIG_WIFI_DISPLAY */
...
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_MIRROR_LINK_IE
	wifi_mirror_link_deinit(global);
#endif /* CONFIG_MIRROR_LINK_IE */
