		...
		for (i = 0; i < wpa_s->wapi_scan_res->num && !selected; i++)
		{
			...
			*/
			...
			for (ssid = group; ssid; ssid = ssid->pnext)
			{
				...
				if (wpa_supplicant_ssid_bss_match(wpa_s, ssid, &wpa_bss)) {
					selected = bss;
					*selected_ssid = ssid;
			...
			if(wpa_s->key_mgmt != ie.key_mgmt) {
				...
				if (wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt)) {
					...
					if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_4WAY_HANDSHAKE))
						wpa_sm_set_pmk(wpa_s->wpa, wpa_s->conf->ssid->psk, PMK_LEN);
				       ...
				       union wpa_event_data *data)
{
	...
	int ft_completed;
#ifdef BRCM_ROAM_OFFLOAD
	int try_okc = wpa_s->current_ssid->proactive_key_caching < 0 ?
		wpa_s->conf->okc : wpa_s->current_ssid->proactive_key_caching;

	wpa_msg(wpa_s, MSG_INFO, "ASSOC INFO: FW 4WAY %d", wpa_s->drv_flags&WPA_DRIVER_FLAGS_4WAY_HANDSHAKE? 1:0);
#endif /* BRCM_ROAM_OFFLOAD */
			      ...
			      struct deauth_info *info)
{
	...
	if (sm->SUPP_BE_state == SUPP_BE_RECEIVE) {
		/*call as if the EAP FAILURE is recv and exit*/
		sm->SUPP_BE_state = SUPP_BE_FAIL;
		enum eapol_supp_result result = EAPOL_SUPP_RESULT_EXPECTED_FAILURE;
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_SCAN_RESULTS:
		if (os_reltime_initialized(&wpa_s->scan_start_time)) {
			...
			if(strncmp(wpa_s->ifname, "wlan0", 3) == 0)
