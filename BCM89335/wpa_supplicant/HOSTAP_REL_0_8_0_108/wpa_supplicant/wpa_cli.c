...
static int wpa_cli_cmd_all_sta(struct wpa_ctrl *ctrl, int argc, char *argv[])
{
	...
	memset(ifname_buff,0,sizeof(ifname_prefix));
	if(ifname_prefix){
		os_snprintf(ifname_buff, sizeof(ifname_buff), "IFNAME=%s ", ifname_prefix);
	}
	os_snprintf(cmd, sizeof(cmd), "%sSTA-FIRST",ifname_buff[0] != '\0' ? ifname_buff : "");
	if (wpa_ctrl_command_sta(ctrl, cmd, addr, sizeof(addr)))
		return 0;
	do {
		os_snprintf(cmd, sizeof(cmd), "%sSTA-NEXT %s",ifname_buff[0] != '\0' ? ifname_buff : "", addr);
...
static int wpa_cli_cmd_mirror_link_ie(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	char cmd[100];
	int res;
	if ((argc != 2 && argc != 3)) {
		printf("Invalid mirror_link ie command: needs one or two "
		       "arguments (subelem, hexdump)\n");
		return -1;
	}
	if (argc > 2) {
	res = os_snprintf(cmd, sizeof(cmd), "MIRRORLINK_IE %s %s %s",
			  argv[0], argc > 1 ? argv[1] : "", argv[2] ? argv[2] : "");
	} else {
	res = os_snprintf(cmd, sizeof(cmd), "MIRRORLINK_IE %s %s",
...
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_MIRROR_LINK_IE
	{ "mirrorlink_ie", wpa_cli_cmd_mirror_link_ie, NULL,
	  cli_cmd_flag_none,
	  "<mirror_link_ie> = <add/del> <subelem_id> <subelem> add mirrorlink ie" },
#endif
#ifdef CONFIG_INTERWORKING
