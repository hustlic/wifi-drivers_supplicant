 ...
 * See README for more details.block_ext_scan
 */
...
static int * freq_range_to_channel_list(struct wpa_supplicant *wpa_s,
					char *val);
extern int wpas_p2p_set_go_passphrase(struct wpa_supplicant *wpa_s,
	...
	if (e->buf[0] == WFD_SUBELEM_DEVICE_INFO) {
		...
		if (0x80 != (WPA_GET_BE16(&e->buf[3]) & 0x80)) {
			...
			ret = wpa_drv_send_tdls_mgmt(wpa_s, broadcast_ether_addr, action_code,
								1, 0, 0, wpabuf_head(wfd_ie), wpabuf_len(wfd_ie));
			if (ret)
...
#ifdef PSET_STA_OPEN_PSK // for get_network psk from wpa_cli
...
static int p2p_ctrl_find(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	else if (os_strstr(cmd, "type=fast_progressive"))
		type = P2P_FIND_FAST_PROGRESSIVE;
#endif
			    ...
			    char *buf, size_t buflen)
{
	...
	ht40 = (os_strstr(cmd, " ht40") != NULL) || wpa_s->conf->p2p_go_ht40 ||
		vht;
	reject = os_strstr(pos, " reject") != NULL;
...
static int wpa_supplicant_iface_create(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	return wpa_drv_if_add(wpa_s, WPA_IF_STATION, ifname, NULL, NULL, NULL,
			mac_addr, NULL);
}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	int reply_len;
	wpa_printf(MSG_ERROR, "[swa]:%s entry %s\n", __FUNCTION__, buf);
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (sdo_running) {
			os_snprintf(temp, sizeof("P2P_SD_FIND"), "%s", "P2P_SD_FIND");
			reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
		} else
#endif
...
#ifdef BCM_MAP_SDCMDS_2_SDOCMDS
		if (sdo_running) {
			os_snprintf(temp, sizeof("P2P_SD_STOP"), "%s", "P2P_SD_STOP");
			reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
		}
		else
#endif
...
#if defined(BCM_SDO) && defined(BCM_MAP_SDCMDS_2_SDOCMDS)
		os_snprintf(temp, sizeof(temp), "%s%s", "P2P_SD_LISTEN ", buf + 11);
		reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
#else
...
#if defined(BCM_SDO) && defined(BCM_MAP_SDCMDS_2_SDOCMDS)
		os_snprintf(temp, sizeof("P2P_SD_LISTEN"), "%s", "P2P_SD_LISTEN");
		reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
#else
...
#ifdef BCM_MAP_SDCMDS_2_SDOCMDS
	if (sdo_running) {
		/* Lets stop the existing request */
		os_snprintf(temp, sizeof("P2P_SD_STOP"), "%s", "P2P_SD_STOP");
		reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
		/* Lets cancel the existing request if any*/
		os_snprintf(temp, sizeof("P2P_SD_CANCEL_REQ"), "%s", "P2P_SD_CANCEL_REQ");
		reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
	}
	os_snprintf(temp, sizeof(temp), "%s%s", "P2P_SD_REQ ", buf + 18);
	reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
	if(reply_len != -1){
		sdo_running = 1;
	}
...
#ifdef BCM_MAP_SDCMDS_2_SDOCMDS
	/* Lets stop the existing discovery */
	os_snprintf(temp, sizeof("P2P_SD_STOP"), "%s", "P2P_SD_STOP");
	reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
	os_snprintf(temp, sizeof("P2P_SD_CANCEL_REQ"), "%s", "P2P_SD_CANCEL_REQ");
	reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
...
#ifdef BCM_MAP_SDCMDS_2_SDOCMDS
	/* So the request must be in below string format for QR pair
	*p2p_service_add upnp 10 uuid:44444444-8574-59ab-9322-022376990ad9::urn:schemas-upnp-org:service:ContentDirectory:1
	*P2P_SD_SVC_ADD upnp 0x10urn:schemas-upnporg:device:InternetGatewayDevice:1
	*0x10uuid:6859dede-8574-59ab-9332-123456789012::urn:schemas-upnporg:device:InternetGatewayDevice:1
	*we need to move 24 bytes to reach pointer to uuid*/
		query_start = os_strstr(buf , "::");
		if (query_start) {
			query_start += 2;
			os_snprintf(temp, sizeof(temp), "%s%s 0x10%s","P2P_SD_SVC_ADD upnp 0x10", query_start, (buf + os_strlen("p2p_service_add upnp 10 ")));
			reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
		}
#else
...
#ifdef BCM_MAP_SDCMDS_2_SDOCMDS
/*we need to move 16 bytes for pointer to name of service to delete */
		os_snprintf(temp, sizeof(temp), "%s%s", "P2P_SD_SVC_DEL ", (buf + os_strlen("P2P_SERVICE_DEL ")));
		reply_len = wpa_supplicant_driver_cmd(wpa_s, temp, reply, reply_size);
#else
        ...
        wpa_printf(MSG_DEBUG, "P2P_SET_GO_PASSPHRASE\n");
        if (wpas_p2p_set_go_passphrase(wpa_s, buf + 22) < 0)
            reply_len = -1;
	...
	} else if (os_strncmp(buf, "MIRRORLINK_IE ", 14) == 0) {
		if (!wpa_s->global->wifi_display) {
			wpa_printf(MSG_ERROR, "%s:Enable WFD prior mirror\n",__FUNCTION__);
			reply_len = -1;
		}
		else if (wifi_mirror_link_ie_set(wpa_s->global, buf + 14) < 0)
			reply_len = -1;
#endif
					   ...
					   char *cmd)
{
...
#ifdef CONFIG_BRCM_VIF
	Boolean add_new_iface = FALSE;

	if ((pos = os_strstr(cmd, "create")) != NULL) {
		add_new_iface = TRUE;
		*pos = '\0';
	}
#endif /* CONFIG_BRCM_VIF */
	...
	if (add_new_iface) {
		...
		wpa_printf(MSG_DEBUG, "Adding interface '%s'", iface.ifname);
		wpa_drv_if_add(global->ifaces, WPA_IF_STATION, iface.ifname, NULL, NULL,
				NULL , mac_addr, NULL);

		wpa_printf(MSG_DEBUG, "New interface '%s' is created with mac addr:" MACSTR,
...
#ifdef CONFIG_BRCM_VIF
	char *pos;
	Boolean delete_iface = FALSE;

	if (os_strstr(cmd, "delete")) {
		delete_iface = TRUE;
		pos = os_strchr(cmd, ' ');
		if (pos)
			*pos = '\0';
...
#ifdef CONFIG_BRCM_VIF
	if (!ret && delete_iface) {
		ret = wpa_drv_if_remove(global->ifaces, WPA_IF_STATION, wpa_s->ifname);
	}
#endif /* CONFIG_BRCM_VIF */
	return ret;
}
					       ...
					       char *buf, size_t *resp_len)
{
	...
	static const char * prefix[] = {
		...
		"P2P_SET_GO_PASSPHRASE ",
#endif
/* Added wfds related cmds to global ctrl iface cmd table */
		NULL
	};
	int found = 0;
	int i;
	...
	for (i = 0; !found && cmd[i]; i++) {
		if (os_strcmp(buf, cmd[i]) == 0) {
			found = 1;
			wpa_printf(MSG_DEBUG, "swa:%sfound:%d \n", __FUNCTION__, found);
		}
	}
					       ...
					       char *buf, size_t *resp_len)
{
	...
	if (os_strncmp(buf, "WFD_SUBELEM_SET ", 16) == 0 ||
	    ...
	    os_strncmp(buf, "MIRRORLINK_IE ", 14) == 0)
		return wpa_supplicant_ctrl_iface_process(global->p2p_init_wpa_s,
							 buf, resp_len);
#endif /* CONFIG_WIFI_DISPLAY */
	return NULL;
						...
						char *buf, size_t *resp_len)
{
	...
	wpa_printf(MSG_ERROR, "[swa]%s:cmd:%s\n", __FUNCTION__, buf);
